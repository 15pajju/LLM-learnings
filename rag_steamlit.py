...
emb_fn = openai_embedding_function.OpenAIEmbeddingFunction
# Suppress tokenizer warnings
os.environ["TOKENIZERS_PARALLELISM"] = "false"
load_dotenv()
API_KEY = "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"
...
def streamlit_app():
    ...
    st.title(":rocket: Space Facts RAG System")
    ...
    with st.expander(":books: Available Space Facts", expanded=False):
    ...
    if query:
        with st.spinner("Processing your query..."):
            ...
            with col1:
                st.markdown("### :robot_face: Response")
                st.write(response)
            with col2:
                st.markdown("### :book: References Used")
                for ref in references:
                    st.write(f"- {ref}")
            # Show technical details in expander
            with st.expander(":mag: Technical Details", expanded=False):
